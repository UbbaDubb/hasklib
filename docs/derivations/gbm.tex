...
interface; the sampler derived below is the \emph{exact} alternative
...
an SDE with \emph{constant} drift and diffusion, which integrates
directly.
...
$f(t,x)=\ln x$, so $f_t=0$, $f_x=1/x$, $f_{xx}=-1/x^2$. With
...
\]
The $X_t$ factors cancel completely --- this cancellation is the whole
point of the $\ln$ substitution --- leaving
...
right-hand side. No time-stepping is needed --- this is what makes it
an \emph{exact} sampler rather than a discretisation, and it is exactly
what \texttt{GBM::sample\_terminal} implements using
\texttt{NormalRng::draw()}.
\end{remark}

\begin{remark}[Where the $-\tfrac12\sigma^2$ comes from]
The drift that appears in \eqref{eq:gbmexact} is \emph{not} $\mu$, the
drift of $X_t$ itself, but $\mu-\tfrac12\sigma^2$. That correction is
exactly the $\tfrac12\diff^2 f_{xx}$ term of It\^o's lemma --- the term
...
is a check on the \emph{sampler}, not on \eqref{eq:gbmsde} itself ---
