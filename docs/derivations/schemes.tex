...
$\drift(\xi,X_\xi)$ with the known left-endpoint value
...
$\diff(t,X_t)(W_{t+\Delta t}-W_t)$ exactly, so freezing it at
the left endpoint is again a leading-order replacement of an unknown
quantity by a known one.

The choice of which endpoint to freeze at is what separates
...
back through $\drift$ and $\diff$ at the start of the next
...
For GBM specifically, \texttt{GBM::sample\_terminal} has zero
...
approximation of the same SDE and is only needed in general
...
therefore return bit-for-bit the same value as
