...
%  (gbm.tex, ou.tex, cir.tex, schemes.tex all cite results here),
...
\texttt{cir.tex}, \texttt{schemes.tex}) cite the results established
...
Riemann--Stieltjes integrals. Its \emph{quadratic} variation, however, is
finite --- the content of \cref{sec:qv} --- and it is precisely this finite
second-order variation that the It\^o integral and It\^o's lemma are built on.
\end{proof}
...
that Brownian motion accumulates \emph{quadratic} variation at a
nonzero rate.
...
\[
   ...
   \qquad \text{in } L^2,
\]
where $\Pi$ is a partition of $[0,T]$. Heuristically this is written
\[
   (\dW)^2 = \dt.
\]
\end{proposition}
...
increments over disjoint intervals are independent. Nothing measure-theoretic
enters beyond these.
...
\[
  \E[S_\Pi]=\sum_{k}\E\big[(\Delta W_k)^2\big]
           =\sum_k \Delta t_k = T ,
\]
which holds for \emph{every} partition, not merely in the limit. Because the
mean is exactly $T$, the mean-square error \emph{is} the variance:
\[
...
\emph{plausible}; the variance computation is what makes it \emph{usable}.
...
\emph{drift} and $\diff$ the \emph{diffusion} coefficient.
\end{definition}
...
\begin{remark}[Correspondence with Brigo's lecture notes]
The lecture notes write the same SDE as
$\dd X_t = f(t,X_t)\dt + \sigma(t,X_t)\dW$; here the drift is $\drift$ and
the diffusion $\diff$, matching the \texttt{StochasticProcess} accessors.
The quadratic variation is written with angle brackets $\langle W\rangle$ in
both (we carry the horizon $T$, the notes use $t$). One symbol clash is worth
signposting: the $C^{1,2}$ test function in It\^o's lemma (\cref{thm:ito}) is
$f$ here but $\phi$ in the notes, whereas the notes' $f$ is our \emph{drift}
$\drift$. The mathematics is identical; only the letters differ.
\end{remark}
...
Let $X_t$ solve \eqref{eq:sde} and let $f(t,x)\in C^{1,2}$. Then
\begin{equation}\label{eq:ito}
   \dd f(t,X_t) =
     \Big( f_t + \drift\, f_x + \tfrac12 \diff^2 f_{xx} \Big)\dt
     \;+\; \diff\, f_x \,\dW,
\end{equation}
...
Expand $f(t+\dt,\,X_t+\dd X)$ about $(t,X_t)$ to second order:
\[
  \dd f = f_t\,\dt + f_x\,\dd X
        + \tfrac12 f_{tt}\,(\dt)^2
        + f_{tx}\,\dt\,\dd X
        + \tfrac12 f_{xx}\,(\dd X)^2
        + \cdots
...
reason:
\[
  \tfrac12 f_{tt}\,(\dt)^2 = 0, \qquad
  f_{tx}\,\dt\,\dd X
     = f_{tx}\big(\drift\,(\dt)^2 + \diff\,\dt\,\dW\big) = 0 .
\]
What remains is
\[
  \dd f = f_t\,\dt + f_x\,\dd X + \tfrac12 f_{xx}\,\diff^2\,\dt .
\]
Substitute $\dd X = \drift\,\dt + \diff\,\dW$ once more and collect the $\dt$
and $\dW$ parts:
\[
  \dd f = \Big( f_t + \drift\, f_x + \tfrac12 \diff^2 f_{xx} \Big)\dt
        + \diff\, f_x \,\dW ,
\]
...
$\dd f = f_t\,\dt + f_x\,\dd X$. The whole gap between stochastic and ordinary
calculus is the single surviving term $\tfrac12\diff^2 f_{xx}\,\dt$, and it
...
\texttt{GBM::sample\_terminal}. That derivation is carried out in
\texttt{gbm.tex}.
\end{remark}
